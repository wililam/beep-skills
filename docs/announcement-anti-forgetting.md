...
**每次交互都必须语音播报！** 这是 OpenClaw Agent 的核心行为准则，所有防遗忘机制都基于此规则。

### 播报类型与正确示例

| 时机 | 类型 | 正确示例（≤20字） | 错误示例（禁止） |
|------|------|-------------------|------------------|
| 收到消息 | `receive` | "收到发布指令" | "我收到了你的指令" ❌ |
| 开始工作 | `task` | "准备上传到GitHub" | "我开始上传" ❌ |
| 执行中 | `task` | "正在生成文档" | "我正在生成" ❌ |
| 任务完成 | `complete` | "上传完成" | "我上传完了" ❌ |
...
**完整规则文档**: `skills/beep-skills/HOOK.md`（安装后必读）
